import { useState } from 'react';
import { InputBox,Some } from './compunent';
import useCurencyinfo from './hook/useCurencyInfo';
function App() {
    let BackgroundImage = 'data:image/jpeg;base64,'
    // the amount usergive 
    const [amount,setAmount] = useState(0)
    // the amount which is converted
    const [convertedAmount,setConvertedAmount] = useState(0);
    // the option of the curent pkr which will be converteing
    const [from,setFrom] = useState("pkr");
    // the option in which we converting the money from (from-to)
    const [to,setTo] = useState('usd');
    // the information of the (from) curency from the custom hook 
    let currencyInfo = useCurencyinfo(from);
    // converting the index of the (from)curency into array
    let options = Object.keys(currencyInfo);
    // swap the the from-to and amout-convertedAmount
    function swap() {
        setFrom(to);
        setTo(from)
        setConvertedAmount(amount);
        setAmount(convertedAmount)
        
    }
    // convert the amount of money and add the contry code 21pkr
    const formatter = new Intl.NumberFormat('en', {
        style: 'currency',
        currency: `${to}`,
        minimumFractionDigits: 0, // Removes .00
    });
    return (
        <div
        className="w-full   h-screen flex flex-wrap justify-center items-center bg-cover bg-no-repeat"
        >
            <div className="w-[50%] h-screen bg-amber-300" style={{backgroundSize:"contain",backgroundRepeat:"unset",backgroundImage:`url('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSOYwzv62KBoXCO25KsZRtCW6X5rUQoXZN15P13Kn0SuwpGmjjO66l18bc&s')`}}>
            </div>
            <div className="w-[50%]  h-screen bg-amber-600 " style={{backgroundSize:"contain",backgroundImage:`url(${BackgroundImage})`}}>
                <div className=" relative top-[30vh] w-full max-w-md mx-auto border border-gray-60 rounded-lg p-5 backdrop-blur-sm bg-white/30">
                    <form
                        onSubmit={(e) => {
                            e.preventDefault();
                           
                        }}
                    >
                        <div className="w-full mb-1">
                            <InputBox
                                label={"from"}
                                currentCurency={from}
                                curencyOptions={options}
                                onCurencyChange={setFrom}
                                amount={amount}
                                setAmount={setAmount}
                                className='transition-all ease-in duration-1000'
                               
                            />
                        </div>
                        <div className="relative w-full h-0.5">
                            <button
                                type="button"
                                className="absolute left-1/2 -translate-x-1/2 -translate-y-1/2 border-2 border-white rounded-md bg-blue-600 text-white px-2 py-0.5"
                                onClick={swap}
                            >
                                swap
                            </button>
                        </div>
                        <div className="w-full mt-1 mb-4">
                            <InputBox
                                label="To"
                                currentCurency={to}
                                curencyOptions={options}
                                onCurencyChange={setTo}
                                amount={convertedAmount}
                                amountDisabled={true}
                                curencyDisabled={false}

                            />
                        </div>
                        <button
                                
                        onClick={()=>setConvertedAmount(prev => (prev=Math.floor(currencyInfo[to]*amount)))}
                        type="submit" className="w-full bg-blue-600 text-white px-4 py-3 rounded-lg">
                            Convert {from} to {to} {formatter.format(convertedAmount)}
                        </button>
                    </form>
                </div>
            </div>
        </div>
    );
}

export default App
